...
def random_mask(image_tensor, mask_ratio=0.5, background_value=0.0):
    c, h, w = image_tensor.shape
    num_elements = h * w
    num_mask = int(num_elements * mask_ratio)

    mask = torch.ones(num_elements, device=image_tensor.device)
    indices = torch.randperm(num_elements, device=image_tensor.device)[:num_mask]
    mask[indices] = 0.0 # Set masked pixels to 0


    mask = mask.view(h, w)
    masked_image = image_tensor * mask.unsqueeze(0) # Apply mask channel-wise

    background_tensor = torch.full_like(image_tensor, background_value)

    final_image = torch.where(mask.unsqueeze(0) == 0, background_tensor, masked_image)
...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    background_value_for_mask = 1.0 if dataset.white_background else 0.0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if args.save_hints_until_iter > 0 and iteration <= args.save_hints_until_iter:
              try:
                  hint_save_dir = os.path.join(scene.model_path, "intermediate_hints", str(viewpoint_cam.uid))
                  os.makedirs(hint_save_dir, exist_ok=True)

                  hint_filename = f"hint_view_{viewpoint_cam.uid}_iter_{iteration}.png"
                  hint_save_path = os.path.join(hint_save_dir, hint_filename)

                  masked_image = random_mask(torch.clamp(image.clone().detach(), 0.0, 1.0),
                                              background_value=background_value_for_mask)

                  # Save the masked image
                  torchvision.utils.save_image(masked_image, hint_save_path)

              except Exception as e:
                  print(f"\n[WARN] Failed to save intermediate hint for iter {iteration}, view {viewpoint_cam.uid}: {e}")
...
if __name__ == "__main__":
    ...
    ### some exp args
    parser.add_argument("--sparse_train_files", type=str, default=None, # Add new argument
                        help="Path to a file containing image filenames (one per line, relative to image dir or absolute) for sparse training. Overrides llffhold/eval split.")
    ...
    parser.add_argument("--save_hints_until_iter", type=int, default=0,
                        help="Save masked intermediate renders up to this iteration (0 to disable).")
    parser.add_argument("--no_mask_loss", action='store_true', default=False,
                        help="Disable masking of the ground truth image for L1/SSIM loss calculation, even if random_background is used.")
