...
def readColmapSceneInfo(path, images, eval, llffhold=8, extra_opts=None):
    ...
    if hasattr(extra_opts, 'sparse_train_files') and extra_opts.sparse_train_files is not None:
        sparse_train_filepath = extra_opts.sparse_train_files
        print(f"Using sparse training files listed in: {sparse_train_filepath}")
        assert osp.exists(sparse_train_filepath), f"Sparse training file list not found: {sparse_train_filepath}"

        # Read desired filenames
        with open(sparse_train_filepath, 'r') as f:
            # Use basename for matching, assuming filenames in the file might not have full paths
            # or might have different relative paths than CameraInfo.image_path
            # Adjust this logic if your file contains full, exact paths matching CameraInfo.image_path
            desired_train_basenames = {osp.basename(line.strip()) for line in f if line.strip()}

        # Filter cameras based on filename
        all_cam_infos = cam_infos # Use the full sorted list before any llffhold/eval split
        
        selected_train_cam_infos = []
        selected_test_cam_infos = []

        for cam in all_cam_infos:
            if osp.basename(cam.image_path) in desired_train_basenames:
                selected_train_cam_infos.append(cam)
            else:
                # Assign all other cameras to the test set
                selected_test_cam_infos.append(cam)

        if len(selected_train_cam_infos) != len(desired_train_basenames):
             print(f"[Warning] Found {len(selected_train_cam_infos)} matching train cameras, but {len(desired_train_basenames)} filenames were listed.")
             # You might want to add more robust error handling here, e.g., list missing/extra files.

        train_cam_infos = selected_train_cam_infos
        test_cam_infos = selected_test_cam_infos # Use all non-training images as test images
        
        print(f"Sparse view by filename: Using {len(train_cam_infos)} images for training and {len(test_cam_infos)} for testing.")
        
        # Ensure eval flag doesn't interfere if sparse files are provided
        eval = False # Override eval flag when using sparse file list

    elif eval: # Keep original llffhold/eval logic if not using sparse file list
         train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
         test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
    else: # Default case: use all for training, none for testing
         train_cam_infos = cam_infos
         test_cam_infos = []
